...
class CallsService {
    ...
    async getCalls(page = 1, pageSize = 20, filters = {}) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to query Cosmos DB for call logs', { error });
            return this.getMockCalls(page, pageSize);
        }
    }
    ...
    async getCallById(id) {
        ...
        try {
            ...
            logger_1.logger.error('Failed to get call from Cosmos DB', { error, id });
            return null;
        }
    }
    ...
    generateMockCalls() {
        ...
        const statuses = ['completed', 'in-progress', 'failed', 'missed'];
        const questions = [
        ...
        return Array.from({ length: 50 }, (_, i) => {
            ...
            return {
                ...
                transcript: [
                    { speaker: 'assistant', text: ['Hindi', 'Marathi', 'Bengali'].includes(lang) ? 'AskBox mein aapka swagat hai! Main aapki kaise madad kar sakti hoon?' : 'Welcome to AskBox! How can I help you today?', timestamp: startTime.toISOString() },
                    { speaker: 'caller', text: question, timestamp: new Date(startTime.getTime() + 5000).toISOString() },
                    { speaker: 'assistant', text: `Great question! Let me explain ${question.toLowerCase().replace('?', '')}...`, timestamp: new Date(startTime.getTime() + 8000).toISOString() },
                ],
                aiResponses: [
